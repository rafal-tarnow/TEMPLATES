...
struct Spring {
	...
	float Ks, Kd;		//stałe sprężystości i tłumienia
};
...
vector<glm::vec4> X;		//bieżące położenie tkaniny
vector<glm::vec4> X_last;	//poprzednie położenie tkaniny
...
LARGE_INTEGER frequency;        //tyknięcia w ciągu sekundy
...
//identyfikatory kwerendy czasowej
GLuint t_query, query;

//czas trwania klatki
GLuint64 elapsed_time;

//zmienne renderingu siatki
...
//tworzy obiekty bufora dla tkaniny i siatki 
void createVBO()
{
	//fill the vertices
	int count = 0;
...
void OnMouseDown(int button, int s, int x, int y)
{
	if (s == GLUT_DOWN)
	{
		oldX = x;
		oldY = y;
		//przekaż współrzędne ekranowe punktu klikniętego
		//zwróć uwagę na odwrócenie kierunku współrzędnej Y
		int winY = (height - y); 
		int winX = x ; 
		float winZ =0;
