...
bool ReadMaterialLibrary(const std::string& filename, vector<Material*>& materials) {
	...
	//analiza składni pliku
	string line;
	Material* pMat = 0;
	while(getline(buffer, line)) { 
		line = trim(line);

		if(line.find_first_of("#") != string::npos)	//to jest komentarz i należy go pominąć
			continue;
		if(line.length()==0)
			continue;
		...
		if(prefix.compare("newmtl") ==0)	//jeśli blok rozpoczyna się przedrostkiem newmtl
		{
			pMat = new Material(); 
...
bool ObjLoader::Load(const string& filename, vector<Mesh*>& meshes, vector<Vertex>& verts, vector<unsigned short>& indices, vector<Material*>& materials, BBox& bbox, vector<glm::vec3>& verts2, vector<unsigned short>& indices2) {
	ifstream fp(filename.c_str(),ios::in);
	if(!fp)
		return false;
    string tmp(std::istreambuf_iterator<char>(fp), (std::istreambuf_iterator<char>()));
	...
	while(getline(buffer, line)) {
		line=trim(line);
		if(line.find_first_of("#") != -1)	//to jest komentarz i należy go pominąć
			continue;
		...
		if(prefix.compare("vt")==0)	//jeśli są współrzędne tekstury
		{
			line = line.substr(space_index+1);
			glm::vec2 uv;
		...
		if(prefix.compare("v")==0)	//jeśli jest wierzchołek
		{
			if(isNewMesh) {
				mesh = new Mesh();
		...
		if(prefix.compare("vn")==0)	//jeśli jest wierzchołek
		{ 
			line = line.substr(space_index+1);
			glm::vec3 v;
	...
	//sortowanie siatek wg materiału 
	for(size_t i=0;i<materials.size();i++) {
	...
	//kopiowanie zawartości do liniowej tablicy bez sortowania wg materiału
	/*
