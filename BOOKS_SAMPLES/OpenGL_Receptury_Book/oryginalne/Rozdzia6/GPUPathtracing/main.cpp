...
//shader renderujący siatkę, shader śledzący ścieżki i shader płaszczyzny
...
void OnInit() {
	...
	//wczytanie shadera śledzenia ścieżek
	pathtraceShader.LoadFromFile(GL_VERTEX_SHADER, "shadery/pathtracer.vert");
	pathtraceShader.LoadFromFile(GL_FRAGMENT_SHADER, "shadery/pathtracer.frag");
	//kompilacja i konsolidacja programu shaderowego
...
void OnRender() {
	...
	//jeśli włączone jest śledzenie ścieżek
	if(bPathtrace) {
		//włączenie shadera śledzenia ścieżek 
		pathtraceShader.Use();
			...
			glUniformMatrix4fv(pathtraceShader("invMVP"), 1, GL_FALSE, glm::value_ptr(invMVP));
				...
				DrawFullScreenQuad();
		//wyłączenie shadera śledzenia ścieżek
		pathtraceShader.UnUse();
	} else {
		//rasteryzacja
...
//obsługa klawiatury w celu przełączania śledzenia ścieżek i rasteryzacji
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Śledzenie ścieżek na GPU - OpenGL 3.3");
