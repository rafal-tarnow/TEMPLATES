...
//shader œwiat³a reflektorowego
GLSLShader shader;
GLSLShader* pFlatShader; // shadera rysuj¹cego siatkê u¿yjemy równie¿ do wskazania po³o¿enia reflektora
...
//w³aœciwoœci œwiat³a reflektorowego
glm::vec3 lightPosOS=glm::vec3(0,2,0); //po³o¿enie œwiat³a w przestrzeni obiektu
glm::vec3 lightPosES;					//po³o¿enie Ÿród³a w przestrzeni oka
glm::vec3 spotPositionOS=glm::vec3(0,1,0);	//po³o¿enie reflektora w przestrzeni obiektu
glm::vec3 spotDirectionES;					//kierunek reflektora w przestrzeni oka
...
void OnInit() {
	...
	//wczytywanie shaderów oœwietlenia reflektorowego na poziomie fragmentów
	shader.LoadFromFile(GL_VERTEX_SHADER, "shadery/SpotLight.vert");
	shader.LoadFromFile(GL_FRAGMENT_SHADER, "shadery/SpotLight.frag");
	...
	shader.Use();
		...
		//przekazanie wartoœci uniformów
	...
	//wyznaczanie po³o¿enia œwiat³a we wspó³rzêdnych sferycznych
	lightPosOS.x = spotPositionOS.x + radius * cos(theta)*sin(phi);
	lightPosOS.y = spotPositionOS.y + radius * cos(phi);
	lightPosOS.z = spotPositionOS.z + radius * sin(theta)*sin(phi);
...
void OnRender() {
	...
	//wyznaczanie kierunków œwiat³a i reflektora w przestrzeni oka przez mno¿enie 
	//przez macierz modelu i widoku
...
void OnMouseWheel(int button, int dir, int x, int y) {
	...
	//wyznaczanie nowego po³o¿enia œwiat³a
	lightPosOS.x = spotPositionOS.x + radius * cos(theta)*sin(phi);
	lightPosOS.y = spotPositionOS.y + radius * cos(phi);
	lightPosOS.z = spotPositionOS.z + radius * sin(theta)*sin(phi);
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Reflektor - OpenGL 3.3");
