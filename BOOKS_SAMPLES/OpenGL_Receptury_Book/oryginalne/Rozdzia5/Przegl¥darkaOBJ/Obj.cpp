...
//wczytanie pliku materiałowego (.MTL)
...
bool ObjLoader::Load(const string& filename, vector<Mesh*>& meshes, vector<Vertex>& verts, vector<unsigned short>& indices, vector<Material*>& materials) {
	ifstream fp(filename.c_str(),ios::in);
	if(!fp)
		return false;
    string tmp(std::istreambuf_iterator<char>(fp), (std::istreambuf_iterator<char>()));
