...
//tekstura (tablica LUT) funkcji przejścia
GLuint tfTexID;
...
//wartości wstępnych kolorów tekstury (tablicy LUT) funkcji przejścia
...
//ta funkcja wczytuje dane wolumetryczne z podanego pliku i umieszcza je teksturze 3D
bool LoadVolume() {
...
//poniższa funkcja generuje interpolowane kolory z zestawu kolorów wstępnych (jet_values)
//najpierw oblicza przyrosty dla poszczególnych składowych
//i odległości wyrażone w indeksach. A następnie interpoluje liniowo sąsiednie kolory wstępne, aby utworzyć 
//kolory pośrednie
void LoadTransferFunction() {
	float pData[256][4];
	int indices[9];

	//wpisywanie kolorów wstępnych w miejsca, w których powinny być po wypełnieniu całej tablicy LUT
	for(int i=0;i<9;i++) {
		int index = i*28;
	...
	//dla każdej pary kolejnych kolorów wstępnych wyznacz różnice wartości RGBA i przeprowadź interpolację
	for(int j=0;j<9-1;j++)
	{
	...
	//przyłączanie tekstury do jednostki 1
...
void SliceVolume() {
	//wyznaczanie największej i najmniejszej odległości każdego wierzchołka jednostkowego sześcianu w kierunku patrzenia
	...
	//zmienne lokalne do przechowywania wektorów początku i kierunku oraz wartości lambda dla punktów przecięcia
	...
	//odjęcie odległości najmniejszej od największej i podzielenie tej różnicy przez całkowitą liczbę płatów
	//w celu wyznaczenia odległości między płatami
	...
	//zaktualizuj obiektu bufora nowymi nwierzchołkami
...
void OnInit() {
	...
	shader.Use();
		...
		//przyłączanie tekstury wolumetrycznej do jednostki 0
		//i tekstury funkcji przejścia do jednostki 1
	...
	//wczytywanie danych wolumetrycznych i generowanie tekstury 3D
	if(LoadVolume()) {
	...
	//wczytywanie funkcji przejścia i generowanie dla niej tekstury (tablicy LUT)
...
void OnRender() {
	...
	glBindVertexArray(volumeVAO);
		//uaktywnienie shadera wolumetrycznego
		shader.Use();
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Cięcie tekstury 3D i klasyfikacja - OpenGL 3.3");
