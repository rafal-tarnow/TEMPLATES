...
int currentFrame = -1; //-1 -> Poza wiązania, 0 do frameCount-1 -> po skinningu
...
//prosta klasa kwaternionów dualnych
class dual_quat {
public:
	glm::quat ordinary, dual;	//dwa zwykłe kwaterniony i ich dualny odpowiednik

	//prosta funkcja tworząca kwaternion dualny  
	//na podstawie orientacji (q0) i translacji (t)
	...
	//konwersja kwaternionu dualnego na macierz
...
//funkcja do generowania transformacji globalnych dla wszystkich kości
//po prostu przebiega przez cały szkielet i mnoży
//transformację lokalną bieżącej kości z globalną transformacją jej rodzica.
...
void OnInit() {
	...
	//zmień rozmiary wektorów pozy wiązania, pozy odwrotnego wiązania, animatedXform i dualQuaternion
	...
	shader.Use();
		...
		//ustalanie wartości uniformów, ponieważ mamy kwaternion dualny, przekazujemy go
		//jako 2 zmienne vec4 i stąd mnożenie przez 2
...
void OnIdle() {
	...
	if(currentFrame == -1) {
		...
		for(int j=0;j<pAnim->mTrackCount;j++) {
			...
			//w metodzie z kwaternionem dualnym wydobądź położenie i orientację, a następnie użyj
			//funkcji QuatTrans2UDQ, aby utworzyć kwaternion dualny dla animatedXform
	...
	//przekaż nowy kwaternion dualny do shadera przez uaktualnienie uniformu
	shader.Use();
		//ponieważ mamy kwaternion dualny, przekazujemy go jako 2 zmienne vec4 
		// i stąd mnożenie przez 2	
		glUniform4fv(shader("Bones"), skeleton.size()*2, &(dualQuaternions[0].ordinary.x));
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Przeglądarka animacji szkieletowych EZMesh (Kwaterniony dualne) - OpenGL 3.3");
