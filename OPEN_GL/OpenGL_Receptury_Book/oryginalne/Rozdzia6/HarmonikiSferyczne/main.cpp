...
//shadery harmonik sferycznych, renderowania siatki i płaszczyzny
...
void OnInit() {
	...
	//wczytanie shaderów harmonik sferycznych
	sh_shader.LoadFromFile(GL_VERTEX_SHADER, "shadery/sh_shader.vert");
	sh_shader.LoadFromFile(GL_FRAGMENT_SHADER, "shadery/sh_shader.frag");
	//kompilacja i konsolidacja programu shaderowego
	...
	shader.LoadFromFile(GL_FRAGMENT_SHADER, "shadery/sh_shader.frag");
	...
	//ustawienie shadera harmonik sferycznych jako bieżącego
		...
		pCurrentShader->Use();
			...
			for(size_t i=0;i<materials.size();i++) {
				...
				if(pMat->map_Kd !="") {
					...
					//pobierz identyfikator aktualnie związanej tekstury i sprawdź, czy różni się od identyfikatora tekstury bieżącej
					//jeśli tak, zwiąż nową teksturę 
					GLint whichID[1];
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Harmoniki sferyczne - OpenGL 3.3");
