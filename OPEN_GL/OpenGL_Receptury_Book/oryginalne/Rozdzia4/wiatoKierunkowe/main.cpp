...
//shader oœwietlenia kierunkowego
GLSLShader shader;
...
//tablica wierzcho³ków odcinka wskazuj¹cego kierunek œwiat³a oraz ID obiektu bufora wiercho³ków
GLuint lightVAOID;
...
glm::vec3 lightDirectionOS = glm::vec3(0,0,0); //kierunek œwiat³a w przestrzeni obiektu
...
//Wspó³rzêdne sferyczne kierunku œwiat³a
float theta = 0.9f;
float phi = 1.37f;
...
void OnMouseMove(int x, int y)
{
	if (state == 0)
		...
		//wyznaczanie wektora kierunku padania œwiat³a
		lightDirectionOS.x = cos(theta)*sin(phi);
		lightDirectionOS.y = cos(phi);
		lightDirectionOS.z = sin(theta)*sin(phi);

		//aktualizacja obiektu bufora dla odcinka symbolizuj¹cego swiat³o
...
void OnInit() {
	...
	//wczytywanie shaderów oœwietlenia kierunkowego
	shader.LoadFromFile(GL_VERTEX_SHADER, "shadery/DirectionalLight.vert");
	shader.LoadFromFile(GL_FRAGMENT_SHADER, "shadery/DirectionalLight.frag");
	...
	//Wyznaczanie wektora kierunku œwiat³a we wspó³rzêdnych sferycznych
	lightDirectionOS.x = cos(theta)*sin(phi);
	lightDirectionOS.y = cos(phi);
	lightDirectionOS.z = sin(theta)*sin(phi);
	
	//vao i vbo dla odcinka symbolizuj¹cego œwiat³o
	glm::vec3 crossHairVertices[2];
	crossHairVertices[0] = glm::vec3(0,0,0);
	crossHairVertices[1] = lightDirectionOS;

	//przygotowanie tablicy wierzcho³ków i obiektu bufora do przechowania kierunku œwiat³a
	//jako odcinka zaczepionego w pocz¹tku uk³adu wspó³rzêdnych
	...
	//pobranie wskaŸnika do shadera siatki potrzebnego do wyrenderowania
	//odcinka
	pFlatShader = grid->GetShader();
	
	cout<<"nicjalizacja powiodla sie"<<endl;
}


//rzwalnianie wszystkich alokowanych zasobów
void OnShutdown() {
	pFlatShader = NULL;
...
void DrawScene(glm::mat4 MView, glm::mat4 Proj ) {
	...
	glBindVertexArray(cubeVAOID); {
		...
		glm::mat4 cM = T;				//macierz modelu
		glm::mat4 cMV = MView*cM;		//macierz modelu i widoku
		glm::mat4 cMVP = Proj*cMV;		//po³¹czona macierz modelu, widoku i rzutowania
		//uniformy shadera
	...
	//wyœwietlanie wektora kierunku œwiat³a
	glBindVertexArray(lightVAOID); {
		//wi¹zanie shadera
		pFlatShader->Use();
			//ustawianie uniformu
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Œwiat³o kierunkowe - OpenGL 3.3");
