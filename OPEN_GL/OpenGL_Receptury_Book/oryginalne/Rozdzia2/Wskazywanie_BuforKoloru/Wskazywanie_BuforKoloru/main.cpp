...
void OnMouseDown(int button, int s, int x, int y)
{
	if (s == GLUT_DOWN)
	{
		...
		//odczytywanie koloru klikniêtego piksela
		GLubyte pixel[4];
		glReadPixels(x, HEIGHT-y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixel);

		//wybieranie kostki na podstawie koloru
		selected_box=-1;
...
void OnInit() {
	...
	//wy³¹czenie ditheringu (wymagane w celu unikniêcia b³êdów przy porównywaniu kolorów)
	glDisable(GL_DITHER);
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Wskazywanie na podstawie koloru - OpenGL 3.3");
